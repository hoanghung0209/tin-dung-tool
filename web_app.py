...
# ==========================================
# LOGIC XỬ LÝ BIẾN ĐỘNG DỮ LIỆU (CALLBACKS)
# ==========================================
...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] { background-color: #F8F9F9; border-radius: 5px 5px 0 0; padding: 10px 20px; }
    </style>
""", unsafe_allow_html=True)

st.title("🏦 HỆ THỐNG KHỞI TẠO HỒ SƠ TÍN DỤNG (ONLINE)")
st.caption("HOÀNG VIỆT HƯNG - QTDND PHÙNG HƯNG")

# ==========================================
# SIDEBAR & QUÉT QR ĐÍCH DANH
# ==========================================
with st.sidebar:
    ...
    if excel_file and qr_img:
        # Đọc dữ liệu để lấy danh sách đối tượng
        with open("temp_data.xlsx", "wb") as f: f.write(excel_file.getbuffer())
        mapping, field_types, tabs_dict = read_mapping("temp_data.xlsx")
        
        # Tự động tìm các "đối tượng" có trong form (Thành viên, Vợ, Chồng...)
        subjects = ["Tất cả"]
        ...
        if st.button("🚀 TIẾN HÀNH QUÉT & ĐIỀN", type="primary", use_container_width=True):
            with open("temp_qr.jpg", "wb") as f: f.write(qr_img.getbuffer())
            try:
                info = parse_cccd_payload(decode_qr_offline("temp_qr.jpg"))
                # Logic điền ĐÍCH DANH
                for ph, ds in mapping.items():
                    d_low = ds.lower()
                    # Chỉ điền nếu thuộc nhóm đối tượng được chọn
                ...
                st.rerun()
            except Exception as e: st.error(f"Lỗi đọc mã: {e}")
...
if excel_file and docx_file:
    ...
    # 2. Xử lý Phương án (Tự động điền hàng loạt)
    if pa_file:
        ...
        with col_pa1:
            selected_pa = st.selectbox("⚡ CHỌN PHƯƠNG ÁN VAY VỐN:", ["-- Trống --"] + list(pa_map.keys()))
        with col_pa2:
            st.write("") 
            if st.button("ÁP DỤNG", type="primary", use_container_width=True):
                if selected_pa != "-- Trống --":
                    ma_pa = pa_map[selected_pa]
                    ...
                    # Ghi đè dữ liệu vào Session State
                    for ph, ds in mapping.items():
                        k_var = var_name(ph).lower()
                        if "tên phương án" in ds.lower(): st.session_state[ph] = selected_pa
                        # Chi phí
    ...
    # 3. Vẽ Tabs & Lưới 2 cột (Tetris Layout)
    st.divider()
    ...
    for tab_name in unique_tabs:
        with tab_obj[tab_name]:
            ...
            for ph, ds in fields:
                ...
                cb_kwargs = {"ph": ph, "d_lower": d_low, "field_types": field_types, "mapping": mapping}
                
                with cols[c_idx]:
                    if is_noicap_desc(d_low) or is_loan_type_desc(d_low):
                        st.selectbox(ds, [""] + (NOICAP_OPTIONS if is_noicap_desc(d_low) else LOAN_TYPE_OPTIONS), key=ph, on_change=process_field_change, kwargs=cb_kwargs)
                    elif is_tall:
    ...
    if st.button("🚀 TẠO HỒ SƠ WORD TỔNG HỢP", type="primary", use_container_width=True):
        ...
        st.download_button("📥 TẢI XUỐNG FILE WORD", data=out_stream, file_name=f"HSCV_{datetime.now().strftime('%H%M')}.docx", type="primary")

else:
    st.info("💡 HƯỚNG DẪN: Tải file Data.xlsx và Mẫu Word (.docx) ở thanh điều khiển bên trái để nạp ứng dụng.")
