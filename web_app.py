...
# ==========================================
# 4. CHỐNG GHI ĐÈ CCCD (KIỂM TRA CHÉO TÊN BIẾN + MÔ TẢ)
# ==========================================
def get_group(ph, ds):
    """
    KIỂM TRA KÉP: Vừa dò Mô tả (ds), vừa dò Tên biến (ph)
    Ngăn chặn tuyệt đối việc Ngày sinh của Đồng vay bị nhầm thành của Thành viên
    """
    d = ds.lower()
    k = var_name(ph).lower()
    text_to_search = f"{d} | {k}" # Gộp cả 2 để quét
    
    if any(x in text_to_search for x in ["thụ hưởng", "thuhuong", "_th"]): 
        return "Người thụ hưởng"
    if any(x in text_to_search for x in ["ủy quyền 2", "uyquyen2", "uq2"]): 
        return "Người ủy quyền 2"
    if any(x in text_to_search for x in ["ủy quyền 1", "ủy quyền", "uyquyen1", "uq1", "uyquyen", "_uq"]): 
        return "Người ủy quyền 1"
    if any(x in text_to_search for x in ["đồng vay 2", "dongvay2", "dv2"]): 
        return "Người đồng vay vốn 2"
    if any(x in text_to_search for x in ["đồng vay", "cùng vay", "dongvay"]) or re.search(r'(?:_dv|dv\d*)$|^dv_', k): 
        return "Người đồng vay vốn 1"
    if any(x in text_to_search for x in ["vợ", "vo", "chồng", "chong", "bảo lãnh", "baolanh"]): 
        return "Khác"
    
    # Nếu không dính bất kỳ từ khóa đuôi nào ở trên, mới là của Thành viên chính
    return "Thành viên"
    ...
    if excel_file:
        ...
        if qr_img and st.button("🚀 TIẾN HÀNH QUÉT", type="primary", use_container_width=True):
            ...
            try:
                ...
                for ph, ds in mapping.items():
                    # CHỐNG GHI ĐÈ BẰNG TƯỜNG LỬA KIỂM TRA CHÉO
...
if excel_file and docx_file:
    # 1. FILL PHƯƠNG ÁN (GỒM CẢ THU NHẬP & CHI PHÍ)
    if pa_file:
    ...
    if st.button("🚀 XUẤT HỒ SƠ WORD", type="primary", use_container_width=True):
        ...
        st.download_button("📥 TẢI FILE WORD", data=out, file_name=f"{member_name}_{datetime.now().strftime('%H%M')}.docx", type="primary")        return final_txt[0].upper() + final_txt[1:] + " mét vuông."
    except: return ""
